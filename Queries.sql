...
ORDER BY ?pred
--------------------------find properties of persons------------
SELECT count distinct ?pred  WHERE {
  ?pred a rdf:Property .
  ?pred <http://www.w3.org/2000/01/rdf-schema#domain> <http://dbpedia.org/ontology/Person>
}
ORDER BY ?pred
--------------------
=== Born in Berlin ======
SELECT * WHERE{
?per a <http://dbpedia.org/ontology/Person>.
?per <http://dbpedia.org/ontology/birthPlace> ?city
}
*--------------------------
prefix dbr: <http://dbpedia.org/resource/>
prefix dbo: <http://dbpedia.org/ontology/>

SELECT DISTINCT ?name ?per
WHERE{
?per dbo:birthPlace dbr:Berlin.
?per dbo:birthName ?name
}
-*********************-------------
<http://dbpedia.org/ontology/Person> <http://dbpedia.org/ontology/birthName> "Abdelmoneim" .

*********************************************
prefix dbr: <http://dbpedia.org/resource/>
prefix dbo: <http://dbpedia.org/ontology/>

#SELECT DISTINCT ?name ?per ?dd
Construct{
?per   dbo:deathDate ?dd.
?per  dbo:birthName ?name
}
WHERE{
?per dbo:birthPlace dbr:Damanhour.
?per dbo:birthName ?name
#Optional{?per dbo:deathPlace dbr:Cairo.}
Optional{?per dbo:deathDate ?dd.}
}select
#-----------------'---------
#list of cities
...
prefix dbr: <http://dbpedia.org/resource/>
 ...
 where {?city dbo:country dbr:Egypt.
?city a dbo:City
}
Order by ?city
...
WHERE {
  { <http://data.semanticweb.org/person/amr-el-abaddi> ?property ?hasValue }
  UNION
  { ?isValueOf ?property <http://data.semanticweb.org/person/amr-el-abaddi> }
}
ORDER BY (!BOUND(?hasValue)) ?property ?hasValue ?isValueOf
##************************* open rep2 *******************************
SELECT DISTINCT ?property ?hasValue WHERE  { <http://data.semanticweb.org/person/amr-el-abaddi> ?property ?hasValue }
===========================================Egypt tripples==================
## types and counts
SELECT  ?type (COUNT(*) as ?cnt) WHERE { ?subj a ?type }
GROUP BY ?type
